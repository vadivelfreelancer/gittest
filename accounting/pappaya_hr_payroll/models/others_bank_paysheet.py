...
class OthersBankPaySheet(models.Model):
    _name = 'others.bank.paysheet'
    ...
    paysheet_line           = fields.One2many('others.bank.paysheet.line','bank_paysheet_id', string='Paysheet Lines')
    type_id                    = fields.Selection([('arrears','Arrears'),('incentive','Incentive'),('advance','Advance'),('final_settlement','Full and Final Settlement')],string='Type')
    ...
    def generate_paysheet(self):
        print (self,self.type_id,"eeeeeeeeeeeeeeee")
        if self.type_id == 'arrears':
            ...
            sequence_no = self.env['ir.sequence'].next_by_code('arrears.paysheet.sequence') or _('New')
        ...
        elif self.type_id == 'incentive':
            print ("eeeee333333333333333333")
            workbook = xlwt.Workbook()
            company_name = xlwt.easyxf('font: name Times New Roman, height 350, bold on; align: wrap on, vert centre, horiz centre;')
            company_address = xlwt.easyxf('font: name Times New Roman, height 230, bold on; align: wrap on, vert centre, horiz centre;')
            header = xlwt.easyxf('font: name Times New Roman, height 200, bold on,italic off; align: wrap on, vert centre, horiz centre;  borders: top thin, bottom thin, left thin, right thin;')
            ...
            sheet_name = 'Incentive Bank Pay Sheet'
            sheet = workbook.add_sheet(sheet_name)
            sheet.row(0).height = 450;
            ...
            if self.month_id:
                ...
                arrears_domain.append(('payment_state','=','pending'))
                arrears_domain.append(('state','=','approve'))
                hr_arrears_sr = self.env['hr.incentive'].search(arrears_domain, order="branch_id")
                
                if not hr_arrears_sr:
                    raise ValidationError(_("No Records found to generate Incentive Bank Paysheet"))
                ...
                row_no += 1
                for arrears_id in  hr_arrears_sr:
                    for arrears_line in self.env['hr.incentive.line'].search([('incentive_id','=',arrears_id.id),('date_from','>=',start),('date_to','<=',end)]):
                        sheet.write(row_no, 0, None, answer); sheet.write(row_no, 1, None, answer); sheet.write(row_no, 2, None, answer); sheet.write(row_no, 3, None, answer);
                        sheet.write(row_no, 4, None, answer); sheet.write(row_no, 5, None, answer); sheet.write(row_no, 6, None, answer); sheet.write(row_no, 7, '%.2f' % arrears_line.approved_amount, answer);
                        sheet.write(row_no, 8, None, answer); sheet.write(row_no, 9, None, answer); sheet.write(row_no, 10, arrears_line.employee_id.name, answer); sheet.write(row_no, 11, arrears_line.employee_id.bank_id.name, answer);
                        sheet.write(row_no, 12, arrears_line.employee_id.ifsc, answer); sheet.write(row_no, 13, arrears_line.employee_id.account_number, answer); sheet.write(row_no, 14, None, answer); sheet.write(row_no, 15, None, answer);
            ...
            sequence_no = self.env['ir.sequence'].next_by_code('incentive.paysheet.sequence') or _('New')
    
            self.write({
                'paysheet_line':[(0, 0, {'bank_paysheet_id':self.id, 'sequence_no':sequence_no, 'dated_on':datetime.now(), 'generated_by':self.env.user.id, 'filedata': data, 'filename': 'Incentive Bank Pay Sheet.xls'})]
            })
            
            print ("111111111111111111111")
        elif self.type_id == 'advance':
            ...
            sheet_name = 'Advance Bank Pay Sheet'
            sheet = workbook.add_sheet(sheet_name)
            sheet.row(0).height = 450;
            ...
            if self.month_id:
                ...
                arrears_domain.append(('payment_state','=','pending'))
                arrears_domain.append(('state','=','approve'))
                hr_arrears_sr = self.env['hr.advance'].search(arrears_domain, order="branch_id")
                
                
                
                if not hr_arrears_sr:
                    raise ValidationError(_("No Records found to generate Advance Bank Paysheet"))
                ...
                sheet.write_merge(0, 0, 0, 8, title , company_address)
                row_no += 1
                for arrears_line in  hr_arrears_sr:
                    #for arrears_line in self.env['hr.arrears.line'].search([('arrears_id','=',arrears_id.id)]):
                    sheet.write(row_no, 0, None, answer); sheet.write(row_no, 1, None, answer); sheet.write(row_no, 2, None, answer); sheet.write(row_no, 3, None, answer);
                    sheet.write(row_no, 4, None, answer); sheet.write(row_no, 5, None, answer); sheet.write(row_no, 6, None, answer); sheet.write(row_no, 7, '%.2f' % arrears_line.advance_amount, answer);
                    sheet.write(row_no, 8, None, answer); sheet.write(row_no, 9, None, answer); sheet.write(row_no, 10, arrears_line.employee_id.name, answer); sheet.write(row_no, 11, arrears_line.employee_id.bank_id.name, answer);
                    sheet.write(row_no, 12, arrears_line.employee_id.ifsc, answer); sheet.write(row_no, 13, arrears_line.employee_id.account_number, answer); sheet.write(row_no, 14, None, answer); sheet.write(row_no, 15, None, answer);
                    sheet.write(row_no, 16, None, answer); sheet.write(row_no, 17, None, answer); sheet.write(row_no, 18, None, answer); sheet.write(row_no, 19, None, answer);
                    sheet.write(row_no, 20, None, answer); sheet.write(row_no, 21, arrears_line.employee_id.work_email, answer); sheet.write(row_no, 22, arrears_line.employee_id.work_mobile, answer); sheet.write(row_no, 23, None, answer);
                    ...
                    sheet.write(row_no, 44, None, answer); sheet.write(row_no, 45, None, answer); sheet.write(row_no, 46, None, answer); sheet.write(row_no, 47, None, answer); sheet.write(row_no, 48, None, answer)
                    row_no += 1
                    arrears_line.payment_state = 'paid'
                row_no += 1
            ...
            sequence_no = self.env['ir.sequence'].next_by_code('advance.paysheet.sequence') or _('New')
    
            self.write({
                'paysheet_line':[(0, 0, {'bank_paysheet_id':self.id, 'sequence_no':sequence_no, 'dated_on':datetime.now(), 'generated_by':self.env.user.id, 'filedata': data, 'filename': 'Advance Bank Pay Sheet.xls'})]
            })
        
        elif self.type_id == 'final_settlement':
            ...
            sheet_name = 'Full and Final Settlement Bank Pay Sheet'
            sheet = workbook.add_sheet(sheet_name)
            sheet.row(0).height = 450;
            ...
            if self.month_id:
                ...
                arrears_domain = []
                employee_active_ids = self.env['hr.employee'].sudo().search([('branch_id','in',branch_sr.ids),('active','=',True)]).ids
                employee_deactive_ids = self.env['hr.employee'].sudo().search([('branch_id','in',branch_sr.ids),('active','=',False)]).ids
                employee_ids = employee_active_ids + employee_deactive_ids
                if employee_ids:
                    arrears_domain.append(('employee_id','in',employee_ids))
                arrears_domain.append(('date_from','>=',start))
                arrears_domain.append(('date_to','<=',end))
                arrears_domain.append(('payment_state','=','pending'))
                arrears_domain.append(('state','=','done'))
                
                hr_arrears_sr = self.env['hr.exit'].search(arrears_domain, order="employee_id")
                if not hr_arrears_sr:
                    raise ValidationError(_("No Records found to generate Full and Final Settlement Bank Paysheet"))
                ...
                for arrears_line in  hr_arrears_sr:
                    #for arrears_line in self.env['hr.arrears.line'].search([('arrears_id','=',arrears_id.id)]):
                    sheet.write(row_no, 0, None, answer); sheet.write(row_no, 1, None, answer); sheet.write(row_no, 2, None, answer); sheet.write(row_no, 3, None, answer);
                    sheet.write(row_no, 4, None, answer); sheet.write(row_no, 5, None, answer); sheet.write(row_no, 6, None, answer); sheet.write(row_no, 7, '%.2f' % arrears_line.slip_id.net_amount, answer);
                    sheet.write(row_no, 8, None, answer); sheet.write(row_no, 9, None, answer); sheet.write(row_no, 10, arrears_line.employee_id.name, answer); sheet.write(row_no, 11, arrears_line.employee_id.bank_id.name, answer);
                    sheet.write(row_no, 12, arrears_line.employee_id.ifsc, answer); sheet.write(row_no, 13, arrears_line.employee_id.account_number, answer); sheet.write(row_no, 14, None, answer); sheet.write(row_no, 15, None, answer);
                    sheet.write(row_no, 16, None, answer); sheet.write(row_no, 17, None, answer); sheet.write(row_no, 18, None, answer); sheet.write(row_no, 19, None, answer);
                    sheet.write(row_no, 20, None, answer); sheet.write(row_no, 21, arrears_line.employee_id.work_email, answer); sheet.write(row_no, 22, arrears_line.employee_id.work_mobile, answer); sheet.write(row_no, 23, None, answer);
            ...
            sequence_no = self.env['ir.sequence'].next_by_code('faf.paysheet.sequence') or _('New')
    
            self.write({
                'paysheet_line':[(0, 0, {'bank_paysheet_id':self.id, 'sequence_no':sequence_no, 'dated_on':datetime.now(), 'generated_by':self.env.user.id, 'filedata': data, 'filename': 'Full and Final settlement Bank Pay Sheet.xls'})]
            })        
...
class ArrearPaysheetLine(models.Model):
    _name = 'others.bank.paysheet.line'

    bank_paysheet_id = fields.Many2one('others.bank.paysheet')

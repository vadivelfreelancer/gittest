...
class StudentAttendanceGenerate(report_sxw.rml_parse):
    ...
    def get_student_name(self, data):
        student = self.pool.get('res.partner').browse(
            self.cr, self.uid, data['student_id'])
        if student:
            return ' '.join([student.name or '',
                             student.middle_name or '',
                             student.last_name or '' ])

    def get_data(self, data):

        sheet_pool = self.pool.get('pappaya.attendance.sheet')
...
class StudentAttendanceReport(models.AbstractModel):
    _name = 'report.pappayaed_attendance.student_attendance_report'
    _inherit = 'report.abstract_report'
    _template = 'pappayaed_attendance.student_attendance_report'
